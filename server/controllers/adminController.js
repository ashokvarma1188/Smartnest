...
const changeUserRole = async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ success: false, message: "User not found" });
    res.status(200).json({ success: true, message: `${user.name} is now ${role}`, user });
  } catch (error) {
    console.log(error);
    res.status(500).json({ success: false, message: "Server Error" });
  }
...
module.exports = { getStats, getAllUsers, getAllPropertiesAdmin, deleteUser, setupAdmin, changeUserRole };
